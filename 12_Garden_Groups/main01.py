...
def check_angle(adj, i,j,k, region):
    ...
    if(cell1[0] != region and cell2[0] != region and cell3[0] != region):
        if(not(cell1[1][0] ==cell2[1][0] == cell3[1][0]) and not(cell1[1][1] ==cell2[1][1] == cell3[1][1])):
            if(not(in_diagonal(adj, i,j,k))):
                return True
    return False
...
def edges(input, region):
    ...
    count = 0
    for cell in area:
        adj = adj_cells(cell[1][0], cell[1][1], input)
        i = 0
        while(i < len(adj)):
            j = (i+1) % (len(adj))
            k = (i+2) % (len(adj))
            ...
            print(adj)
            if(check_angle(adj, i,j,k, region)):
                count+=1
            i+=1
